...
> 定向修复、概念归并、运行清单六项能力。核心取舍(agent 不写文件、先 MD 后切章、
> SQLite 只做索引)保持不变。
...
### 3.1 先转 Markdown,再按章节切分

系统前期不做 RAG chunk,先保留完整章节语境。
...
原因:章节正文、summary、quiz 和 cards 都需要完整章节内容。过早切成小块会丢掉章节结构。

### 3.2 agent 返回内容,整合器写文件

内容 agent 不写最终 Markdown 文件,只返回结构化结果。整合器负责把这些结果写入 Markdown。这样可以减少并发冲突,也可以保证
Markdown 格式稳定。

### 3.3 chunk 只服务 RAG

chunk 不参与前期内容生成,只在最终 vault 生成后出现。

```text
输入: vault/chapters/*.md  vault/concepts/*.md  vault/sources/*.md
输出: site/.bookwiki/bookwiki.sqlite
```

### 3.4 SQLite 是索引,不是内容源

Markdown 是内容源,SQLite 是检索库和结构化缓存。网站正文从 Markdown 读取;搜索、Quiz、Cards 和问答从 SQLite 读取。

### 3.5 调度即 DAG,而非阶段(v2 新增)

生成过程不是"一个阶段做完再做下一个阶段",而是一张任务依赖图。每个 agent 调用是图上的一个任务节点,节点之间只声明依赖关系。一个全局
worker 池按"依赖就绪 + 优先级"执行任务,任意一章的轻任务可以填补另一章重任务留下的空闲。这样做的结果是:worker
不会因为等待某个"阶段"结束而空转,生成可以断点续跑、可以增量重建。详见 §8、§11。

### 3.6 章节结构由 AI 设计,人工复核后锁定(v2 新增)

章节骨架(有哪些章、什么顺序)不照搬任何一本教材的目录,而是由 **StructureAgent 读完所有资料摘要后,按"怎样学最顺"重新设计**
。原因:教材未必按利于学习的顺序编排,而且当输入是多本教材、教材 + 课件混合、甚至质量参差时,根本不存在一个现成可照搬的目录。让
AI 综合所有来源、重排出一套教学结构,比硬挑一本书当骨架更合理。

但 AI 设计结构有两个风险:可能排错,而且每次跑结果可能不一样(不可复现)。**因此结构设计这一步是一道硬性人工复核 checkpoint:
** AI 产出一份"建议结构 + 来源片段→章节"的映射草案,人在这里审阅、修改、确认后锁定,系统才继续往下烧 LLM。锁定后的结构存盘,后续可复现。

这里也是保证"考试对齐"的地方——AI 重排可能偏离老师的讲授/考核顺序,由复核的人(老师或学生)在 checkpoint
上把结构拉回到既好学、又对得上考试的状态。这个取舍贯穿 §6(配置)、§7.3(片段归属)、§8(流程与闸门)、§10.2(StructureAgent)。

> **脊柱是一个"被设计并被批准"的决定,不是某份输入的固有属性。** 所有资料(讲得好的、讲得烂的、不同版本的、PPT、试卷)
> 都是被对齐到这根已批准脊柱上的素材。
...
| 章节切分模块     | 把 source Markdown 切成 chapter source |
...
注意:`role` 只影响**冲突时的取舍和呈现**,不决定章节骨架——骨架由 StructureAgent 综合所有来源设计(§3.6)。
...
| `review`   | `required`(默认)/ `skip`      | 是否在结构落地前强制人工复核 checkpoint(§8.3) |
...
章节资料包不是"把整份文件塞进某一章",而是**把每份资料切成带 `source_ref` 的片段,再按主题对齐到已批准结构(§3.6)的章节上**
...
- **置信度 + 报告**:每条归属带置信度,汇总进 `chapter-split-report.md`(来源 × 章节对齐矩阵 + 未归属清单),正是 §8.3 结构
  checkpoint 上人工要审的对象。
...
每个结果旁边还有一份缓存元数据 `work/.cache/{task_id}.meta.json`,用于断点续跑(见 §11.4)。
...
- **结构 checkpoint**:StructureAgent 产出建议结构 + 片段映射草案后,人在此审阅、修改、锁定(`structure.review: required`
  时强制,§8.3)。锁定前不进入任何章节生成,不烧后续 LLM。
- **StructureAgent**(barrier):等所有 `SourceSummary` 完成,综合全部来源摘要设计章节骨架与顺序。用摘要而非全文做对齐,既省又准。
- **ConceptReconcile**(barrier):等所有 `ConceptExtract` 完成,做跨章概念去重(见 §13)。
...
| 章节切分 | source Markdown                 | `chapter_sources/chxx` | 章节切分     |
| 资料摘要 | source Markdown                 | source summary         | Agent 生成 |
| 大纲生成 | source summaries                | outline                | Agent 生成 |
| 章节生成 | chapter source(+ outline)       | chapter result         | Agent 生成 |
...
§8.1 的 DAG 描述的是 **generate 这一段内部** 的并行结构。在更外层,整条流水线按"阶段"组织:**convert → split → generate →
check → index**。这里有一个明确的设计选择:**阶段之间通过磁盘文件通信,不靠内存对象传递。**

这样做的直接好处是,同一套代码既能逐段跑、也能一键跑完,不必二选一:

- 阶段产物全部落盘:`sources_md/`、`chapter_sources/`、`agent_results/`、`vault/`、`bookwiki.sqlite`。
- 每个阶段是独立 CLI 命令,可以单独运行、单独看输出(见 §18)。
- `build` 把这些命令串起来,配合任务缓存(§11.4),中途停了 `--resume` 接着跑。
- 这也是 5 人并行开发的前提:每人拿固定的中间文件,单独测自己那一段。

**复核闸门只设在"便宜到改、贵到晚发现"的阶段,不是每个阶段都卡。** 经验上人工只值得卡前两道:

| 阶段边界            | 是否设人工闸门   | 原因                                                       |
|-----------------|-----------|----------------------------------------------------------|
| convert 之后      | **建议设**   | PDF/PPT 提取最脏(断页、页眉页脚、source_ref 错乱),错了污染全部下游;改起来便宜,晚发现极贵 |
| split 之后        | **建议设**   | 章节边界靠启发式判,容易切错;ch03 串了内容,ch03 五个 agent 全白跑;纠正只是改文件归属     |
...
换句话说,人工闸门本质是一道**省钱闸门**:确认源头(convert / split)干净了,再放 LLM 下去烧 token。最贵的 generate
反而交给自动检查器(§14)兜底,而不是人。

实现上:`build` 默认一键贯通;通过 `--pause-after` 在指定阶段后暂停,等人工确认 `chapter-split-report.md`
等中间产物后再继续(见 §18)。团队约定第一次跑某本书时先手动走 `convert`、`split` 两步,确认无误后再 `generate` 往后一键。

```mermaid
flowchart LR
    CV[convert] --> G1{{人工闸门<br/>看 sources_md}}
    G1 --> SP[split]
    SP --> G2{{人工闸门<br/>看 split-report}}
    G2 --> GEN[generate]
    GEN --> CK[check 自动复核]
    CK --> G3{{轻量:只看 NEEDS_HUMAN}}
    G3 --> IDX[index] --> ST[site]
```
...
| ChapterSplitAgent  | 判断章节边界 | source Markdown、目录、标题       | chapter map        |
| OutlineAgent       | 生成全书大纲 | source summaries            | outline JSON       |
| ChapterAgent       | 生成章节正文 | chapter source(+ outline)   | chapter JSON       |
...
class Task:
    ...
    type: TaskType  # SOURCE_SUMMARY|OUTLINE|CHAPTER|SUMMARY|QUIZ|CARD
...
| 内容质量      | JSON 合法但检查器判定缺来源等 | 交给 §14 的定向修复处理,不在此重试                              |

前两类是 v1 完全缺失的兜底,实跑中绝大多数失败属于这两类。

### 11.6 可观测性:运行清单与 dry-run
...
结果:一个概念一个页面、反向链接一致、不重复做功。这一步同时解决了 §14 里 `[[概念]]` 双链检查的时序问题——概念清单先定,章节里的双链才有目标。
...
bookwiki split-chapters books/ai-intro     # sources_md/   -> work/chapter_sources/
...
因为阶段之间只靠磁盘文件通信(§8.3),所以可以在任意阶段停下来人工复核中间产物,确认无误再跑下一段。

### 18.2 一键命令与人工闸门

总命令按顺序运行 convert → split-chapters → generate → check → index:

```bash
bookwiki build books/ai-intro
```

`build` 默认一键贯通。若要在便宜且关键的阶段后停下人工复核,用 `--pause-after`:

```bash
# 第一次跑某本书:先到 split 为止,人工确认 sources_md 和 split-report 后再继续
bookwiki build books/ai-intro --pause-after convert,split

# 确认无误后,从 generate 往后一键跑完(命中缓存,convert/split 不重算)
bookwiki build books/ai-intro --resume
```

推荐工作流:**第一次跑先卡 `convert`、`split` 两道闸门**(源头干净再放 LLM 下去烧 token),后续迭代直接 `build --resume`
。generate 这一段不设人工闸门,由检查器自动复核(§8.3、§14)。
...
验收:普通文字版 PDF 与 PPTX 能转 Markdown;每页/每张幻灯片有 `source_ref`;输出能被章节切分模块读取。

### 20.3 成员 C:章节切分 + 内容 Agent

负责 ChapterSplitAgent、OutlineAgent、ChapterAgent、ConceptExtract/Reconcile、ConceptAgent,以及主要内容 prompt 与输出
schema。
...
产出 `work/chapter_sources/ch01/`、`agent_results/ch01.*.json`、`vault/chapters/ch01-xxx.md`。
目标:能切章节、ChapterAgent 返回正文、整合器写入 vault;**A 同步搭起 DAG + worker 池骨架,先支持任务缓存与重试**。
...
运行 split-chapters,展示 chapter_sources
...
BookWiki 是一个面向单本教材的 AI 学习网站生成系统。系统先把 PDF、PPT、试卷和笔记转换为带来源标记的 Markdown,再按章节切成
chapter source。一个调度器把后续所有 agent 工作组织成任务 DAG,用全局 worker 池执行,支持断点续跑、按任务重试、速率与成本控制。多个内容
