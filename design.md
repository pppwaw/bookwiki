...
books/
  ai-intro/
    ...
    work/
      ...
      skeleton.json                   # SkeletonExtractAgent + SkeletonFoldAgent 流式产出的全书术语/别名/章节 brief 契约
...
split 之后先由流式 skeleton 抽取+折叠（SkeletonExtractAgent 分章并行抽候选 → SkeletonFoldAgent 按章序串行折叠归并）生成全书只读术语契约,随后每章按 section 并行生成正文、段级知识题、应用题、卡片、总结和概念候选。**
...
flowchart TD
    ...
    ALN --> D[章节资料包]
    D --> SK[SkeletonExtractAgent + SkeletonFoldAgent<br/>流式 glossary + alias_map + chapter_briefs + chapter_uses]
...
- **build_skeleton**(barrier):在 `split` 后、`generate` 前流式构建全书契约:pass 1 `SkeletonExtractAgent` 分章并行抽取概念候选(源先按 heading 切块),pass 2 `SkeletonFoldAgent` 按章序串行折叠,用确定性 `Registry` reducer 应用 add/alias/rename/merge/split ops,产出 `work/skeleton.json`(canonical glossary、每个概念的 first-owning chapter、`alias_map`、`chapter_briefs`、`chapter_uses`)。`generate` 给每章注入该契约的切片(`alias_map_slice` + `chapter_owns` + `chapter_uses`),从源头统一术语并提供相邻章节过渡线索。
...
| 全书骨架 | 所有 chapter source              | `work/skeleton.json`   | SkeletonExtractAgent + SkeletonFoldAgent |
...
flowchart LR
    ...
    G3 --> SK[SkeletonExtractAgent + SkeletonFoldAgent · 流式全书契约]
...
| SkeletonExtractAgent + SkeletonFoldAgent | 流式生成全书术语骨架 | 所有 chapter source (chunked) | glossary / alias_map / chapter_briefs / chapter_uses |
...
```mermaid
flowchart LR
    S[SkeletonExtractAgent + SkeletonFoldAgent<br/>流式 glossary + alias_map + chapter_uses] --> A[ConceptExtract<br/>每章并行 · 廉价/规则]
...
1. **build_skeleton** node:在章节生成前流式构建:pass 1 `SkeletonExtractAgent` 分章并行抽概念候选(源先按 heading 切块),pass 2 `SkeletonFoldAgent` 按章序串行折叠,用确定性 `Registry` reducer 应用 ops,产出 canonical glossary、每个概念的 first-owning chapter、`alias_map`、`chapter_briefs` 和 `chapter_uses`,写入 `work/skeleton.json`。
...
负责 StructureAgent、ChapterSplitAgent、SkeletonExtractAgent、SkeletonFoldAgent、SectionPlannerAgent、SectionAgent、ConceptExtract/Reconcile、ConceptAgent,以及主要内容 prompt 与输出
schema。
...
目标:StructureAgent 能先根据全部资料摘要重组大纲,人工复核并锁定后再切章节;流式 skeleton 抽取+折叠建全书契约;Section pipeline 返回正文、整合器写入 MDX content;**A 同步搭起 LangGraph 顶层流水线 + 共享 litellm Router 骨架,接入 instructor 和 per-task `task_key` 缓存**。
