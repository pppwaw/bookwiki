...
| reconcile 之后    | **建议设**   | LLM 模糊合并易错(中英混杂、近义、缩写);把不同概念合并 → 所有概念页 + 所有章节双链全错;`concepts.reconciled.json` 一个文件,改起来便宜 |
...
`convert`、`structure`、`split`、`reconcile` 四道闸门,确认无误后再让 generate / concept_pages 往后一键跑完。
...
python scripts/run.py books/ai-intro --pause-after convert,structure,split,reconcile
