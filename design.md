...
系统最后从生成好的 Obsidian vault 构建 SQLite 检索库。Next.js + Fumadocs 网站读取 Markdown 渲染 wiki 页面,并读取 SQLite
...
`conversion.pdf` 控制 PDF 解析。PDF 默认使用 MinerU 的 VLM 模式,解析质量或版面还原优先;`pipeline` 只作为降级选项,用于 VLM 不可用或成本/速度压力较大的场景。
...
PDF 统一交给 MinerU 解析,默认使用 VLM 模式。转换模块接收 MinerU 产物,再做三件事:
...
| false | 仅依赖本章资料包,大纲只用于 vault 的 index 与交叉链接 | prompt 更短     | 跨章一致性弱一些          |
...
| `generate`  | `integrate`(包含 `generate` → `reconcile_concepts` → `concept_pages` → `integrate` 4 个 node) | `work/agent_results/` + `concepts.reconciled.json` + `vault/` |
| `check`     | `check`                                                     | `work/logs/check-report.{md,json}` |
| `repair`    | `repair`(回路 `repair → integrate → check`)                  | 受影响章节的 vault 文件更新 |
| `index`     | `index`                                                     | `site/.bookwiki/bookwiki.sqlite` |

`generate` 这个用户视角阶段是 "agent 跑完 + 概念归并 + 概念页生成 + 整合到 vault" 一气呵成,因为这四步对外只交付一个东西:**一个可被 `check` 检查的 vault**;中间任何一步停下,vault 都不完整,审了也没意义。
...
| `reconcile_concepts` 之后 | **建议设** | LLM 模糊合并易错(中英混杂、近义、缩写);把不同概念合并 → 所有概念页 + 所有章节双链全错;`concepts.reconciled.json` 一个文件,改起来便宜 |
...
g.add_node("integrate",           integrate_node)           # 渲染 vault Markdown,应用 alias_map
...
async def generate_chapter(payload):
    ...
    # 只产 agent_results JSON,不写 Markdown — Markdown 由顶层 integrate node 渲染
...
**整合器为什么必须在 concept_pages 之后**:整合器渲染章节 Markdown 时要按 `alias_map` 规范化双链(§11.2),而 `alias_map` 是 `reconcile_concepts` 的产物;同时章节 Markdown 里还要嵌入概念页链接,所以也得等 `concept_pages` 跑完。**`generate_chapter` 里只产 JSON、不写 Markdown** — 写 vault Markdown 的活全部归顶层 `integrate` node。
...
ChapterAgent 跑的时候 `reconcile_concepts` 还没发生,各章写出来的双链可能用各章自己的命名;归名变化由整合器统一对齐,**ChapterAgent 不必感知 `reconcile_concepts` 的存在**。
...
第一版用 FTS5 做搜索和 RAG 候选召回,向量检索后续加入。网站是一次性生成、生成后不再变动,所以 indexer 走"读 vault 全量重建 SQLite"的简单路径,不做增量比对——这跟 §10.6 的 `input_hash` 缓存只在 vault 层之前生效不冲突。
...
python scripts/generate.py   books/ai-intro                    # 跑到 integrate node 停;产 agent_results/ + concepts.reconciled.json + vault/
...
验收:vault 能构建数据库(全量重建,不需要增量);网站能读取 vault 与 SQLite;搜索/Quiz/Cards/问答可用且问答显示来源;前端代码不含
API key。
