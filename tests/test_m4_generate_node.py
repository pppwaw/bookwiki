...
@pytest.mark.asyncio
async def test_generate_node_feeds_topics_and_figures_to_lesson_prompt(tmp_path) -> None:
    book_dir = tmp_path / "book"
    source_path = book_dir / "work" / "chapter_sources" / "chapter-1" / "source.md"
    source_path.parent.mkdir(parents=True)
    source_path.write_text(
    ...
    runtime = RecordingRuntime([_lesson_response(), _summary_response()])
    cfg = BookConfig(
        book_dir=book_dir,
        book_id="book",
        title="Book",
    ...
    lesson_prompt = runtime.calls[0]["user"]
    # Topics from the approved structure must reach the lesson prompt.
    assert '"topics"' in lesson_prompt
    assert "State space search" in lesson_prompt
    # Figures parsed from the chapter source must reach the lesson prompt.
    assert '"figures"' in lesson_prompt
    assert "paper-p001-b001" in lesson_prompt
    assert "Search tree diagram" in lesson_prompt
    # Option B isolation: the summary agent must not receive the figures payload.
    summary_prompt = runtime.calls[1]["user"]
    assert '"figures"' not in summary_prompt
    assert '"topics"' not in summary_prompt
