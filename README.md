...
- 一套 Obsidian 风格的 **Fumadocs MDX vault**(`<book>/content/docs/`):分章正文、概念页、习题、记忆卡片、章节小结。
...
正如 SurviveSJTUManual 所言，国内大学老师的质量不能说是卓越无比，只能说是天差地别，更多的时候我还是习惯通过PPT和课本自学。恰逢人工智能导论结课，有一项“制作AI驱动的应用”的大作业，我便借此机会编写了 BookWiki，使用AI将PPT和书籍内容结构化，转换为可互动的学习资源。
...
需要 **uv**(Python 依赖与虚拟环境)、**pnpm**(站点构建,并用它安装 Node)、**Node.js ≥ 20**(内置 MDX 校验器 + 学习站点)。均用官方推荐方式,顺序:先装 pnpm,再用 pnpm 装 Node。
...
uv python install 3.12   # 可选;uv sync 也会按需自动装满足 >=3.12 的 Python

# pnpm —— 官方独立脚本(自带运行时,无需预装 Node)
curl -fsSL https://get.pnpm.io/install.sh | sh -

# Node.js —— 用 pnpm 安装并全局启用 LTS(满足 ≥ 20)
pnpm env use --global lts
```

**Windows(PowerShell,`pnpm env --global` 需以管理员身份运行)**
...
# Node.js —— 用 pnpm 安装并全局启用 LTS
pnpm env use --global lts
```

> `pnpm env` 要求 pnpm 由**独立脚本**安装(而非 npm/Corepack,否则循环依赖),目前仍属实验特性。独立脚本**不支持 Intel Mac(darwin-x64)**——Intel Mac 请改从 [nodejs.org](https://nodejs.org/en/download) 直接安装 Node。
...
- **站内搜索**(`/api/search`)与 **RAG 聊天检索**共用一套 SQLite 混合检索:**trigram 中文关键词** + **bge-m3 语义向量** 两路 **RRF** 融合。向量走 OpenRouter(构建期把每个 chunk 的向量算好存进 `bookwiki.sqlite`,查询期算 query 向量)。**没有 embedding key 时自动降级为纯中文关键词检索**(给已有书补语义,需带 `OPENROUTER_API_KEY` 重跑 `index` 阶段)。
- **RAG 聊天**(`/api/chat`):可引用单个概念/章节整页;用 `BOOKWIKI_CHAT_*` 配置模型;检索自动获得上面的语义能力。
...
- `budget.maxCostCny`:成本硬上限,代码默认 `70.0`(示例书设为 `150.0`);`<= 0` 为不限,越线抛 `BudgetExceeded`。每次运行后的实际花费写入 `work/logs/run-manifest.json` 的 `llm_usage.total_cost_cny`,分阶段明细见 `llm_usage.stages`。
  - 经验值:一本中等规模正式教材(约 600–900 页 PDF)跑一次平均约 **¥70**,成本几乎全在 `generate`(每节约 ¥0.9)与 `concept_pages` 两个阶段。
...
| `OPENROUTER_API_KEY`                                                                   | `openrouter-*` 模型;默认视觉图注 `openrouter-qwen3.6-35b-a3b`;**构建期站内搜索 embedding**(可选,缺省则只建 trigram 关键词索引、无语义) |
| `*_API_BASE_URL`                                                                       | 用代理 / 兼容网关时覆盖各家 Base URL                                                                                                   |
| `MINERU_BACKEND` / `MINERU_API_*` / `MINERU_MODEL_VERSION`                             | `convert` 阶段 MinerU 解析后端(本地或云端)配置                                                                                         |
| `BOOKWIKI_CHAT_API_KEY` / `BOOKWIKI_CHAT_BASE_URL` / `BOOKWIKI_CHAT_MODEL`             | 站点 RAG 聊天;查询期站内搜索 embedding 也复用这套凭据                                                                                  |
| `BOOKWIKI_EMBED_MODEL`                                                                 | 站内搜索 embedding 模型(默认 `baai/bge-m3`)。**构建期与查询期必须一致**,否则向量不可比、语义失效;换模型需重跑 `index`                  |
| `BOOKWIKI_EMBED_BASE_URL`                                                              | 构建期 embedding 的 Base URL(默认 OpenRouter `https://openrouter.ai/api/v1`);查询期 Base URL 复用 `BOOKWIKI_CHAT_BASE_URL`             |
...
缺密钥时各处都**直接报错**,不会用占位内容兜底。**唯一例外**:站内搜索的语义路在 embedding 不可用时降级为 trigram 中文关键词检索(关键词是真实词法结果,非伪造内容),搜索不中断。
