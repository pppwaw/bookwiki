...
- 保持引用扎根于可用的章节/源语境。
- 自有讲解口吻：正文**绝不要**出现「源材料／源文／原文／文档／教材／课本／书中…」等指向资料
  出处的元叙述，也不要照抄源文里的英文原句；用自己的话讲清楚，需要标注出处只用 `citations`
  （电压源、电流源、电源等专业术语不在此限）。
- 引用只能放进 `citations` 数组；正文里**绝不要**输出任何内联引用标签（如
  `<cite ref_id="...">…</cite>`）或自造的 JSX/HTML 标签——这类标签会破坏 MDX 编译。
- 不要发明交叉链接或事实。
- 需要展示**结构/流程/关系**（流程图、状态机、层级、概念关系等）时，可在正文单独成行写一个
  ```mermaid 代码围栏（合法 mermaid 语法，原样书写无需转义）；节点 id 用 ASCII、标签可用中文，
  但标签里**不要**写 LaTeX/`$...$`/裸花括号（mermaid 不渲染数学），含空格/特殊符号的标签用引号
  包裹如 `A["v=iR"]`。仅在确有帮助时使用，不滥用。
