...
- `title`：章节标题。**每个标题都必须以 `Chapter N` 开头**（N 为整数），格式如
  "Chapter 6 Point Estimation" 或 "Chapter 6: Point Estimation"，否则下游 split 会报错。
  - 源文本明确含章节编号时，沿用该编号（如 "Chapter 9 ..."），允许编号不连续
    （如缺第 8 章可从 7 跳到 9）。
  - 源文本**没有**可见编号时（如合并章、习题册、附录），也必须补一个唯一的 `Chapter N`
    前缀，N 取紧接前一章之后、且全书不重复的整数。
  - N 在全书内必须唯一，不得有两章共用同一编号。
