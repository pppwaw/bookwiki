...
=== 例题讲解（从课本/PPT 的例题中取材，融入正文）===
- 当源文（课本/PPT）里有**例题 / Example / 例 X.Y / 解答 / 典型计算**时，优先挑最能体现方法的
  一到两道，作为"例题讲解"完整融入正文，而不是只丢一个结论。这是把源文的题目转化为带推导的
  示范，是本工具的核心价值之一。
- 讲解结构（费曼式）：先用自己的话交代**题目情境与已知条件** → **分步**展示解题思路与推导，
  每一步点出用到的定义/定律/公式（如"由欧姆定律 $v=iR$ 得…"）→ 给出**结论**并一句话点评
  （物理意义或常见易错点）。关键步骤不要跳。
- 复述而非照搬：用你自己的话和目标语言复述题目与解法，遵守前述"不照搬源文英文原句、不写
  「源材料中…」"的规则；所有数值与式子用 LaTeX。可适当改写题面措辞、补全省略的中间步骤。
- **与测验题分开（避免泄题，关键）**：被你写成"例题讲解"、已给出完整解法的题目，**不要**再就
  同一道题埋应用题占位 `<QuizItemSlot>` 或出知识题；反过来，准备留给学生做的题，不要在正文里
  提前把答案/完整解法讲出来。例题讲解是"示范"，测验题是"练习"，两者应取**不同的题或不同的数值**。
- 用 `citations` 标注例题出处（`ref_id` 来自已有 <chunk ref="...">）。
- 不滥用：讲透一两道有代表性的例题，胜过罗列一堆；与本段 `learning_goal` 无关的例题不要硬塞。
...
**严禁**把 `<QuizBlock>`/`<QuizItem>`/`<QuizItemSlot>` 或任何 MDX 组件放进 ``` 代码围栏
（包括 ```quiz、```jsx、```mdx 等）——测验标签必须作为 MDX **直接裸写**；代码围栏只允许用于
上面说的 ```mermaid 结构图。
